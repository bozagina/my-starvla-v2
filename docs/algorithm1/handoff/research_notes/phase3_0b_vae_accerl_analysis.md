...
- **训练目标**：最大化 ELBO = $\mathbb{E}*{q*\phi(z|x)}[\log p_\theta(x|z)] - D_{KL}(q_\phi(z|x)  p(z))$
...
| 机制                        | VAE                                                                 | 说明              |
| ------------------------- | ------------------------------------------------------------------- | --------------- |
| 摊销推断（Amortized Inference） | 编码器一次前向即输出 $q_\phi(z                                                | x)$ 的参数         |
| KL 正则                     | 约束 $q_\phi(z                                                        | x)$ 接近先验 $p(z)$ |
...
| 添加 KL 正则 | 对 dynamic_embedding 添加 $D_{KL}(q(z                     | x) | \mathcal{N}(0,I))$      | 嵌入空间更结构化、防塌陷                         |
