...
| `risk_score` | $\max(\text{delta\_norm})$ | 风险强度 |
| `correction_mask` | delta\_norm ≥ Q75 阈值 | 哪些时间步需要纠偏 |
| `affected_region_prior` | delta\_norm 的归一化分布 | 受影响区域的软分布 |
...
$$
\mathcal{L}_{corr} = \lambda_1 \mathcal{L}_{\Delta} + \lambda_2 \mathcal{L}_{trig} + \lambda_3 \mathcal{L}_{smooth}
$$
