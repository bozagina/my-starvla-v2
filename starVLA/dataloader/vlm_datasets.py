...
def make_vlm_dataloader(cfg):
    ...
    model_type = str(getattr(data_args, "model_type", "")).lower()
    if model_type not in {"qwen2.5vl", "qwen3vl", "qwen2vl"}:
        raise ValueError(
            f"Unsupported datasets.vlm_data.model_type={model_type!r}; "
            "qwen_only repo expects one of {'qwen2.5vl','qwen3vl','qwen2vl'}."
        )

    base_vlm = getattr(getattr(cfg.framework, "qwenvl", None), "base_vlm", None)
    if not base_vlm:
        raise ValueError("Missing `framework.qwenvl.base_vlm` for VLM dataloader.")

    processor = transformers.AutoProcessor.from_pretrained(base_vlm, trust_remote_code=True)
    image_processor = processor.image_processor
    tokenizer = processor.tokenizer
    tokenizer.model_max_length = int(data_args.model_max_length)
