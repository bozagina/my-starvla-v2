...
RUN_ID="ah4_v091_500step_$(date +%Y%m%d_%H%M%S)"
OUTPUT_DIR="${REPO_ROOT}/results/Checkpoints/${RUN_ID}"
mkdir -p "${OUTPUT_DIR}"
cp "$0" "${OUTPUT_DIR}/"

CORRECTION_JSONL="${REPO_ROOT}/results/PseudoLabels/p3_0b_outcome_v091/correction_dataset_with_a_outputs.jsonl"

echo "=== AH-4 500-step training ==="
echo "RUN_ID: ${RUN_ID}"
