...
###############################################################################
# CP-AH-4 Corrective Flow Training
#
# From 30k pre-trained QwenPI checkpoint, trains with A-module (lite mode)
# + corrective_loss + CorrectiveFlowHead.
# Protocol: cp_res_ah4_eval_protocol.md
###############################################################################
...
RUN_ID="cp_ah4_cf_3k_$(date +%Y%m%d_%H%M%S)"
...
echo "=== CP-AH-4 Corrective Flow Training (A-module + CF head) ==="
echo "RUN_ID: ${RUN_ID}"
echo "PRETRAINED: ${PRETRAINED_CKPT}"
echo "STEPS: 30000→33000 (3000 additional steps)"
...
stdbuf -oL -eL accelerate launch \
  ...
  --framework.corrective_flow.trigger_threshold 0.5 \
