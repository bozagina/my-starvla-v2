...
###############################################################################
# CP-AH-4-v1 / CP-AH-5 Fusion CF (RTC) Training
#
# Used for both:
#   CP-AH-5: RNG alignment check (single seed, CF training ON but CF inference OFF)
#   CP-AH-4-v1: Multi-seed CF eval (3 seeds, CF inference ON)
#
# RFC v1.1 RTC paradigm:
#   - BD-7: self-attention only (use_cross_attention=false)
#   - BD-8: trigger/region NOT input to CF model
#   - ENG-3: CF RNG alignment via independent Generator
#
# Usage:
#   SEED=42 bash tools/handoff/run_cp_rtc_fusion_cf_train.sh
###############################################################################
...
RUN_ID="cp_ah4_fusion_cf_rtc_seed${SEED}_$(date +%Y%m%d_%H%M%S)"
...
echo "=== CP-AH-4-v1 Fusion CF (RTC v1.1, seed=${SEED}) ==="
echo "RUN_ID: ${RUN_ID}"
